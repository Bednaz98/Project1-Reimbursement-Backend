...
export default class DAOCheckManager implements DAOCheck{
    ...
    async IsAdmin(EmployeeProfile:Profile): Promise<boolean> {
        if(Number(EmployeeProfile?.EmployeeArray?.length) >0){
                if( EmployeeProfile?.ManagerID == undefined){this.DebugLog.print(`No Admin Account exist ${EmployeeProfile.id} `);return false}
                else{await this.DebugLog.print('Admin access');return true}
            }
